...
class MERL:
	...
	def train(self, gen, test_tracker, collisions_tracker=None):
		...
		if gen % self.args.test_gap == 0:
			...
			test_fits = entry[1][0]
			best_distance = mod.list_mean(entry[1][2])
			final_distance = mod.list_mean(entry[1][3])
			test_tracker.update([mod.list_mean(test_fits), best_distance, final_distance], self.total_frames)
...
if __name__ == "__main__":
	...
	test_tracker = utils.Tracker(args.metric_save, [args.log_fname, '_best_distance'+args.log_fname, '_final_distance'+args.log_fname], '.csv')  # Initiate tracker

...
def rollout_worker(args, id, type, task_pipe, result_pipe, data_bucket, models_bucket, store_transitions, random_baseline):
    ...
    while True:
        ...
        best_distance = [None for _ in range(NUM_EVALS)]
        final_distance = [None for _ in range(NUM_EVALS)]
        ...
        while True: #unless done
            ...
            if sum(done)==len(done):
                ...
                for i, grew in enumerate(env.global_reward):
                    fitness[i] = sum(grew) / len(grew)
                    best_distance[i] = max(grew)
                    final_distance[i] = grew[-1]
       ...
       # if type == "test" and (args.config.env_choice == 'rover_tight' or args.config.env_choice == 'rover_loose' or args.config.env_choice == 'motivate'or args.config.env_choice == 'rover_trap'or args.config.config == 'simple_spread'):
        ...
        result_pipe.send([teams_blueprint, [fitness,num_collisions, best_distance, final_distance], frame])
